...
def test_handoff_completes_on_resolution_and_consumes_notify_refs() -> None:
    state = HumanHandoffState(
        title="Escalate",
        requires=[Requirement(field="resolved")],
        notify={"who": Ref(field="agent")},
    )
...
def test_every_state_satisfies_the_engine_contract() -> None:
    states: list[State] = [
        ...
        ActionExecuteState(title="a", requires=[Requirement(field="result")]),
        HumanHandoffState(title="h", requires=[Requirement(field="resolved")]),
    ]
    for state in states:
